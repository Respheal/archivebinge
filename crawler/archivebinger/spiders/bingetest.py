...
class BingetestSpider(scrapy.Spider):
    name = "bingetest"
    handle_http_status = ['303', '302', '301']

    def __init__(self, starturl, cid, count=0, *args, **kwargs):
        super(BingetestSpider, self).__init__(*args, **kwargs)
    ...
    def parse(self, response):
        ...
        if comic['crawler']['position'] == "inner":
            finderstring = '//a[contains(@{0}, "{1}")]/@href'.format(
                    comic['crawler']['tag'],
                    comic['crawler']['identifier']
                    )
            next_page = str(
                    response.xpath(
                        finderstring
        ...
        if next_page:
            ...
            realpage = "{uri.scheme}://{uri.hostname}{uri.path}{uri.query}".format(uri=makeurl)
            print realpage
